...
namespace NodePhysics2D.Demo {
	...
	public class TriangleObject : MonoBehaviour {
		...
		void Start() {
			...
			_left = RotateVector2(Vector2.up, 120f * Mathf.Deg2Rad);
			_right = RotateVector2(Vector2.up, -120f * Mathf.Deg2Rad);
		...
		private Node2D[] MakeStep(int step, Node2D[] prevStepNodes) {
			...
			for (int i = 0; i < step; ++i) {
				...
				_sim.MakeSpring(nodes[i], prevStepNodes[i], _power);
				_sim.MakeSpring(prevStepNodes[i], nodes[i + 1], _power);
				_sim.MakeSpring(nodes[i + 1], nodes[i], _power);
			}
			return nodes;
		}
